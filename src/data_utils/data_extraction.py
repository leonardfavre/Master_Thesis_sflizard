...
def extract_annotation_patches(annotation_file, annotations, patch_size, patch_step):
    ...
    for key, value in tqdm(inst_map_dict.items(), desc="Extracting annotation patches", position=1, leave=False):
...
def extract_patches(array, array_name, patch_size, patch_step):
    ...
    if array.shape[0] % patch_step != 0 and array.shape[1] % patch_step != 0:
        ...
        if patch.shape[0] == patch_size and patch.shape[1] == patch_size:
            array_dict[array_name + f"_{array.shape[0] - patch_size}_{array.shape[1] - patch_size}"] = patch
...
def extract_data(args):
    ...
    for folder in args.images_path:
        ...
        for image_file in tqdm(image_files, desc=f"Extracting images from folder {folder}"):
            # extract patches from the images to have multiple images of same size:
            images.update(extract_image_patches(image_file, args.patch_size, args.patch_step))
    ...
    annotations = pd.DataFrame(columns=["id", "inst_map", "class_map", "nuclei_id", "classes", "bboxs", "centroids"])
    ...
    for folder in args.matlab_file:
        ...
        for annotation_file in tqdm(annotation_files[:5], desc=f"Extracting annotations from folder {folder}", position=0):
            # extract patches from the annotations to correspond to the images patches of same size:
            annotations = extract_annotation_patches(annotation_file, annotations, args.patch_size, args.patch_step)
    ...
    print(f" > Number of nuclei : {np.sum([np.max(d) for d in cleaned_data['annotations']['nuclei_id']])}")
    print(f" > Number of classes : {len(np.unique(np.concatenate([d for d in cleaned_data['annotations']['classes']])))}")
