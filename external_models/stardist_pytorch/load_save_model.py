...
def save_model(
    ...
    **kwargs,
    ...
    if testAcc_to_file is not None:
        testacc_filename = (
            "Testacc_" + stage + Model_name + "_" + Train_mode + "_" + Dataset + ".csv"
        )
    ...
    if testloss_to_file is not None:
        testloss_filename = (
            "Testloss_" + stage + Model_name + "_" + Train_mode + "_" + Dataset + ".csv"
        )
    ...
    if trainAcc_to_file is not None:
        trainacc_filename = (
            "Trainacc_" + stage + Model_name + "_" + Train_mode + "_" + Dataset + ".csv"
        )
    ...
    param_filename = (
        "Parameters_" + Model_name + "_" + Train_mode + "_" + Dataset + ".txt"
    )
    ...
    thefile.write("%s \n" % stage)
    thefile.write(
        "Patience_scheduler=%s,  Weight_decay=%s  \n" % (Parameters[2], Parameters[3])
    )
    if not Parameters[1][0][1:] == Parameters[1][0][:-1]:
        for i in range(len(Parameters[1][0])):
            thefile.write(
                "Initial learning rate for param_groups %s is %s epochs \n"
    ...
    for epoch, lr in zip(Parameters[0], Parameters[1][1:]):
        thefile.write(
            "In epoch %s, maximum of the learning rates decreased to %s \n"
            % (epoch, lr)
        )
    ...
    if testAcc_to_file is not None:
        ...
        if i + 1 == len(testAcc_to_file):
            i = -1
        thefile.write(
            "Maximum test accuracy in epoch %s (if 0  it means that the initial state was the best)\n\n"
            % str(i + 1)
        )
...
def checkpoint_save_stage(
    model,
    trainloss_to_file,
    testloss_to_file,
    train_metric_to_file,
    test_metric_to_file,
    Parameters,
    Model_name,
    Train_mode,
    ...
    for epoch, lr in zip(Parameters[0], Parameters[1][1:]):
        thefile.write(
            "In epoch %s, maximum learning rate decreased to %s \n" % (epoch, lr)
        )
    if not (Parameters[0] == []):
        thefile.write("Trained for %s epochs \n" % Parameters[0][-1])
    thefile.write("\n\n")
