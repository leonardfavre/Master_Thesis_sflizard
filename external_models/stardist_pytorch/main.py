...
Trainloader, Testloader = dataloader.getDataLoaders(
    N_RAYS, max_dist=65, root_dir=DSB2018_PATH
)
...
kwargs = {}
additional_notes = (
    "Star Distance training with distances capped to 65.Rays"
    + str(N_RAYS)
    + ".\
...
RESULTS_DIRECTORY = (
    os.getcwd() + "/" + dataset + "/" + train_mode + "_" + model_name + "/plots/"
)
loss = MyL1BCELoss()
...
optimizer = torch.optim.Adam(
    model.parameters(), lr=init_lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=5e-5
)
scheduler = ReduceLROnPlateau(
    optimizer, "min", factor=0.5, verbose=True, patience=6, eps=1e-8, threshold=1e-20
)
