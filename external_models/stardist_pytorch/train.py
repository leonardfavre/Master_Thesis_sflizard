...
class Trainer:
    def __init__(
        self, loss=None, metric=None, log_dir=None, validate_every=1, verborrea=True
    ):
        self.loss_ce = loss
        self.metric = metric
    ...
    def Train(
        ...
        if scheduler is None and epochs is None:
            ...
            scheduler = ReduceLROnPlateau(
                optimizer, "min", verbose=True, patience=10, eps=1e-8
            )
        path_checkpoint = (
            os.getcwd()
        ...
        if isinstance(scheduler, ReduceLROnPlateau):
            ...
            print(
                "Scheduler not supported. But training will continue if epochs are specified."
            )
            if epochs == None:
        ...
        ]  # first list for epochs, second for learning rate,3rd patience, 4th weight_decay,5 for time
        ...
        while keep_training:
            ...
            if epochs != None:
                if self.verborrea:
                    print(
                        "Epoch {}/{},  lr={}. patience={}, weight decay={}".format(
                            epoch,
                            epochs,
                            max_lr,
                            scheduler.patience,
            ...
            train_loss, train_metric = self.train_scratch(
                model, TrainSet, optimizer, epoch
            ...
            if epoch % self.validate_every == 0:
                ...
                if isinstance(scheduler, ReduceLROnPlateau):
                    ...
                    if (
                        ...
                        and prev_num_bad_epochs == scheduler.patience
                        and not save
                    ):
                        max_lr, list_lr = self.update_list_lr(optimizer)
    ...
    def train_scratch(
        self, model, DataSet, optimizer, epoch
    ):  # eval is not correct in the method
        _loss = 0
        _correct = 0
        model.train()
        kwargs = {}
        for batch_idx, data in enumerate(DataSet):
            ...
            if self.tb_logger is not None:
                ...
                self.tb_logger.log_scalar(
                    tag="train_Loss", value=total_loss.item(), step=step
                )
                ...
                if step % log_image_interval == 0:
                    self.tb_logger.log_image(
                        tag="train_Input", image=inputs[0, 0].to("cpu"), step=step
                    )
                    if distances.dim() == 3:
                    ...
                    self.tb_logger.log_image(
                        tag="train_Target_Affinity",
                        image=distances.to("cpu"),
                        step=step,
                    ),
                    if isinstance(prediction, list):
                        prediction = prediction[1]  # take object probabilities
                    self.tb_logger.log_image(
                        tag="train_Prediction_Affinity",
                        image=prediction[0, 0].to("cpu").detach(),
                        step=step,
                    )
        ...
        with torch.no_grad():
            ...
            if epoch is not None:
                ...
                if self.tb_logger is not None:
                    ...
                    self.tb_logger.log_scalar(
                        tag="val_loss", value=_loss_average, step=step
                    )
                    if self.metric is not None:
                        self.tb_logger.log_scalar(
                            tag="val_metric", value=_acc.item(), step=step
                        )
                    # we always log the last validation images
                    self.tb_logger.log_image(
                        tag="val_Input", image=inputs[0, 0].to("cpu"), step=step
                    )
                    if distances.dim() == 3:
                        distances = distances[0]
                    else:
                        distances = distances[0, 0]
                    self.tb_logger.log_image(
                        tag="val_Target_Affinity", image=distances.to("cpu"), step=step
                    )
                    if isinstance(prediction, list):
                        prediction = prediction[1]
                    self.tb_logger.log_image(
                        tag="val_Prediction_Affinity",
                        image=prediction[0, 0].to("cpu").detach(),
                        step=step,
                    )
