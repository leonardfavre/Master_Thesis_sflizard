...
class MaskStardist:
    ...
    def eval(
        ...
        for i in range(tensors.shape[0]):
            star_labels.append(
                self.compute_star_label(tensors[i], dist[i], prob[i], multiple)
            )
        return star_labels
    ...
    def compute_star_label(
        ...
        """Compute the stare label of an image according dist and prob."""
        ...
        star_label = polygons_to_label(
            dists, points, image.squeeze(0).squeeze(0).shape, probs
        )
        if not multiple:
